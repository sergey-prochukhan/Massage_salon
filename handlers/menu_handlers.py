...
Я — виртуальный помощник массажного салона [Бот_Спа_Резорт]. 
Готов помочь вам:
-подобрать подходящую процедуру;
-познакомить с нашими специалистами;
-записаться на сеанс;
    ...
    #Обработка кнопки "Подробнее"
        ...
        media = InputMediaPhoto(
            ...
            caption=(
                ...
                " - Представленный функционал может быть переработан под нужды любого бизнеса\n"
                " - Модуль оформления заявки реализован с записью в БД и оперативной отправкой информации о заявке в приватную группу\n"
                " - Код этого бота написан на языке Python с использованием библиотеки aiogram\n"
                " - Его можно найти в репозитории https://github.com/PS-Bot-Maker/Massage_salon.git \n"
                " - Бот живёт на хостинге Bothost.ru \n"
                " - Все персонажи адреса и контакты вымышлены, любое совпадение случайно.\n"
                " - Весь медиа-контент создан нейросетями Яндекс\n"
                " - Разработчик: @salebot_se"
            )
        )
        try:
            await callback.bot.edit_message_media(
